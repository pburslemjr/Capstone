...
class CTF(gym.Env):
	...
	def reset(self):
		...
		for i in range(len(gameConsts.players)):
			#if(gameConsts.players[i]['color'] == "blue"):
			...
			'''elif(random.random() < 0.5):
				...
				x = random.randint(200, gameConsts.MAP_WIDTH)
				y = random.randint(200, gameConsts.MAP_HEIGHT)
				flag = Flag(gameConsts.players[i]['color'], (x, y), gameConsts.FLAG_SIZE)'''
	...
	def step(self, actions):
		...
		rew = 0.0#self.reward_model.predict(self.obs)
		#pg.display.flip()
		self.obs = self.observation()
	...
	def handleHit(self, o1, o2):
		...
		if(isinstance(o1, Bullet) and o2Tank and (o1.color != o2.color or gameConsts.FRIENDLY_FIRE) and o1.tank_num != o2.tank_num):
			...
			if(o2.color == "blue"):
				ply_order = [0, 1]
				for i in range(len(self.allFlags)):
					if(self.allFlags[i].color == "blue"):
						for j in range(len(self.gameObjects)):
							ob = self.gameObjects[j]
							if(isinstance(ob, Flag) and ob.color == "blue"):
								self.gameObjects[j] = Flag(gameConsts.players[i]['color'], (gameConsts.players[ply_order[i]]['base']['x'], gameConsts.players[ply_order[i]]['base']['y']), gameConsts.FLAG_SIZE)
					else:
						for j in range(len(self.gameObjects)):
							ob = self.gameObjects[j]
							if(isinstance(ob, Flag) and ob.color == "red"):
								if(random.random() < 0.5):
									self.gameObjects[j] = Flag(gameConsts.players[i]['color'], (gameConsts.players[ply_order[i]]['base']['x'], gameConsts.players[ply_order[i]]['base']['y']), gameConsts.FLAG_SIZE)
								else:
									x = random.randint(200, gameConsts.MAP_WIDTH)
									y = random.randint(200, gameConsts.MAP_HEIGHT)
									self.gameObjects[j] = Flag(gameConsts.players[i]['color'], (x, y), gameConsts.FLAG_SIZE)
