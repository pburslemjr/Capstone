...
class CTF(gym.Env):
    ...
    def __init__(self):
        ...
        self.obs_size=4*(4*6+2+2) #4 total tanks: x, y, velX, velY, rot ??
        self.episode=0
    ...
    def step(self, actions):
        ...
        for obj1 in self.gameObjects:
            for obj2 in self.gameObjects:
                ...
                if(math.sqrt(pow(obj1.position[0] - obj2.position[0], 2) + pow(obj1.position[1] - obj2.position[1], 2)) < max(obj1.size[0], obj1.size[1])/2 + max(obj2.size[0], obj2.size[1])/2 + 10):
                    ...
                    for i, rew in enumerate(hit_rewards[0]):
                        local_rewards[i] += rew
        ...
        print("return of reward: " + str(self.reward(local_rewards, global_rewards)))
        local_rews = self.reward(local_rewards, global_rewards)

        return [self.obs, np.sum(local_rews), done, {}]
    ...
    def reward(self, local_reward, global_reward):
        ...
        for i in range(len(local_reward)):
            r[i] += local_reward[i] + global_reward
        return r
