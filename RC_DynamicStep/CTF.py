...
class CTF(gym.Env):
    ...
    def __init__(self):
        ...
        #An int to track what episode the last phasing happened
        self.last_phase_episode = 0

        #An int to track last best phasing episode reward
        self.last_phase_reward_avg = 50
        ...
        #An array to store the last so many unshaped rewards
        self.prev_unshaped_rews = []

        #A float to store the current episodes cumulative unshaped reward
        self.cumm_unshaped_rew = 0.0
    ...
    def step(self, actions):
        ...
        if(self.time_steps == 20000):
            ...
            self.prev_unshaped_rews.append(self.cumm_unshaped_rew)
            if (len(self.prev_unshaped_rews) == 101):
                self.prev_unshaped_rews.pop(0)
            self.cumm_unshaped_rew = 0.0
        ...
        self.cumm_unshaped_rew += global_rewards
    ...
    def change_rew_frac(self, episode):
        ...
        if (episode > 101):
            print("Average unshaped Reward of last 100 episodes: " + str(sum(self.prev_unshaped_rews)/len(self.prev_unshaped_rews)) + "\n")

        # 150 is about 0.073 average unshaped rewards per step
        # and "> 20" ensures that after a phase, there is at least 200 episodes that pass before
        # it can try to phase again
        if (episode > 101 and ( sum(self.prev_unshaped_rews)/len(self.prev_unshaped_rews) ) >= (self.last_phase_reward_avg + 30) and episode - self.last_phase_episode > 200):
            self.last_phase_episode = episode
            self.last_phase_reward_avg = sum(self.prev_unshaped_rews)/len(self.prev_unshaped_rews)
            self.rew_frac -= 0.1
