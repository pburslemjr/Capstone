...
class CTF(gym.Env):
	...
	def __init__(self):
		...
		#Local reward scale factor
		self.local_reward_scale_factor = 1.5
	...
	def step(self, actions):
		...
		return [self.obs, self.reward(local_rewards, global_rewards), done, {}]
	...
	def phaseLocalReward(self, lr):
		scale_factor = -math.atan(0.001 * self.episode) + (math.pi/2)
		scaled_reward = scale_factor * lr
		self.local_reward_scale_factor = scale_factor
		#print('the currnt sclae factor is: ' + str(scale_factor))
			...
			if(o1.tank_num <= len(self.selectableTanks)):
				if(o1.color != o2.color):
					if(isinstance(o2.flag, Flag)):
						local_rewards[o1.tank_num-1] = local_rewards[o1.tank_num-1] + self.phaseLocalReward(20.0)	#Reward for shooting down enemy carrying your flag
					else:
						local_rewards[o1.tank_num-1] = local_rewards[o1.tank_num-1] + self.phaseLocalReward(10.0)	#Reward for shooting enemy without a flag
				else:
