...
class CTF(gym.Env):
	...
	def step(self, actions):
		...
		#provide reward for closness to ememy flag
		for flag in self.allFlags:
			#skip flags that are not red
			if (flag.color != 'red'):
				continue
			#disregard flags that are picked up by any tank
			elif (flag.pickedUp == True):
				continue
			#elif (flag.pickedUpBy.color == 'red'):
			#	continue
			else:
				blue_tank_x = self.selectableTanks[0].position[0]
				blue_tank_y = self.selectableTanks[0].position[1]
				flag_x = flag.position[0]
				flag_y = flag.position[1]
				flag_dist = self.get_dist(blue_tank_x, blue_tank_y, flag_x, flag_y)
				local_rewards[self.selectableTanks[0].tank_num-1] = local_rewards[self.selectableTanks[0].tank_num-1] - 0.00003 * flag_dist
				#print('flag distance is: ' + str(flag_dist))
