                ...
                with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
                    ...
                    for cyc in range(total_cycles):
                        ...
                        for update in range(1, n_updates+1):
                            ...
                            # Unpack
                            obs, returns, masks, actions, values, neglogpacs, states, ep_infos, true_reward, shaped_rew, unshaped_rew, rew_frac = rollout
                            self.values = values
                            ...
                            if self.verbose >= 1 and (update % log_interval == 0 or update == 1):
                                ...
                                print("Reward Fraction: " + str(rew_frac))
                                ...
                                f.write(str(np.mean(true_reward)) + " " + str(np.mean(shaped_rew)) + " " + str(np.mean(unshaped_rew)) + " " + str(rew_frac) + "\n")
                                f.close()
...
class Runner(AbstractEnvRunner):
    ...
    def _run(self):
        ...
        return mb_obs, mb_returns, mb_dones, mb_actions, mb_values, mb_neglogpacs, mb_states, ep_infos, true_reward, shaped_rew, unshaped_rew, rew_fraction
